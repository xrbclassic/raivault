...
export class RaiPipe implements PipeTransform {
  ...
  transform(value: any, args?: any): any {
    ...
    switch (denomination.toLowerCase()) {
      default:
      case 'xrbc': return `${(value / this.mrai).toFixed(6)}${!hideText ? ' XRBC': ''}`;
      case 'mnano':
        ...
        if (hasRawValue) {
          ...
          return `${this.toFixed(newVal, this.precision)}${!hideText ? ' XRBC': ''}`;
        } else {
          return `${(value / this.mrai).toFixed(6)}${!hideText ? ' XRBC': ''}`;
        }
