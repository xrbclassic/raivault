...
export class ManageWalletComponent implements OnInit {
  ...
  async exportAddressBook() {
    ...
    const exportUrl = `https://wallet.xrbclassic.com/import-address-book#${base64Data}`;
  ...
  exportAddressBookToFile() {
    if (this.walletService.walletIsLocked()) return this.notifications.sendWarning(`Wallet must be unlocked`);
    const fileName = `RaiVault-AddressBook.json`;
  ...
  exportToFile() {
    ...
    const fileName = `RaiVault-Wallet.json`;
    ...
    reader.onload = (event) => {
      ...
      try {
        ...
        if (!importData.length || !importData[0].account) {
          return this.notifications.sendError(`Bad import data, make sure you selected a RaiVault Address Book export`)
        }
